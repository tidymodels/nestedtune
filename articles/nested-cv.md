...
The order of the `metrics` set matters. tune’s three selectors choose on
the first metric in the set, and the outer loop scores every metric in
it. If the first metric is not the one you read, the run chooses under
one loss and assesses under another. For example,
`metric_set(mae, rmse)` chooses by `mae` and also reports `rmse`. Stone
(1974, p. 116) says that the two losses need not match. The run below
passes no set, so it uses tune’s default for a regression,
`metric_set(rmse, rsq)`, and chooses by `rmse`.
