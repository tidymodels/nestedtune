...
[`vignette("estimate")`](https://nestedtune.tidymodels.org/articles/estimate.md)
says. For a workflow chosen before seeing these estimates, the model to
deploy is `nested_final_fit(mapped, id = "forest")`, and that workflow’s
rows in `collect_metrics(mapped)` are the numbers to report for it.
