...
### D-064 (2026-09-16): IP2's text names the randomness it binds, the clause D-011's Consequences and the reproducibility help template already carry. Supersedes no entry

**Context:** IP2 in `cairn/DESIGN.md` promises the same result for the same seed across worker counts and serial or parallel execution, and disclaims identity across R versions, platforms, and tune versions, but does not say which randomness it covers. D-011's Consequences paragraph states the bound (randomness flowing through R's RNG; kernlab's SVM and keras/torch outside it under any R-side scheme), and RR01 finding B4 (`cairn/reviews/archive/RR01-rng-streams-outer-folds.md`) asked that DESIGN.md say so beside IP2's existing disclaimers. The help template `man-roxygen/section-reproducibility.R` carries the same sentence as its last paragraph.
**Decision:** IP2's entry gains one sentence: it binds randomness that flows through R's generator, and engines that randomize outside it (kernlab's SVMs, the deep-learning engines) are outside its reach under any R-side scheme. The seeding scheme and the template's wording are unchanged.
**Consequences:** a reader of DESIGN.md learns IP2's scope without opening D-011. A test on an engine that bypasses R's RNG stays out, since there is nothing to assert. Falsified by an R-side mechanism that pins such an engine, which would narrow the exclusion.
