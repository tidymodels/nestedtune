...
### D-059 (2026-09-07): a `nested_results_set` keeps its class through an operation exactly when every row left is one of the run's own workflows — extends to the set the invariant rule D-031 fixed for `nested_results`, and departs from its row clause

**Context:** D-031 fixed for `nested_results` that a verb may reorder rows and add columns, and that adding or removing a row returns a bare tibble, because a fold table's record describes one design and fewer rows than the design's cannot answer for it (IP4). D-058 made the set a tibble of `wflow_id`, `workflow` and `result` with one `nested_results` per row and registered no subsetting method, so every subset keeps the class: a column subset prints a run it does not hold, an empty subset reaches the readers' all-failed refusal, and a repeated row stacks one workflow's metrics twice under its id (M71 review O2, O3; `cairn/milestones/M73-set-invariants.md` holds the measurements).

**Decision:** the set's record is per row. An operation keeps the class and the `fn` attribute when the result holds the three record columns under their names with none repeated, at least one row, no repeated `wflow_id`, and each row's three values identical to the first data-frame argument's row of that id; otherwise it returns a bare tibble. A dropped or reordered workflow keeps the class, where D-031 sheds it, because each row's `nested_results` describes its own run whole; a repeated row, an added row, a swapped `result` or an empty set sheds it. Five doors route to the one rule — `dplyr_reconstruct()`, `[`, `vec_restore()`, base `rbind()` and `names<-` — as the single class's do. No `vec_ptype2`/`vec_cast` lattice is registered: a set combined with a bare table sheds through both doors under this rule, and a direct `vctrs::vec_cbind()` gives a bare tibble, documented. Considered and rejected: a structural check with no template comparison (binds two map runs into one set, but passes a row whose `result` was swapped for another run's); mirroring D-031's row clause (a `filter()` dropping a failed workflow before reading, the use that asked for this, would return a tibble the readers refuse).
...
### D-062 (2026-09-11): `debias_estimate()` is not exported here, and tune is named as its home. Applies D-002's boundary and GP5 to issue #95's offer

**Context:** Issue #95 (topepo) offers `debias_estimate()`, the Tibshirani and Tibshirani (2009) correction of a tuning run's best score as coded on finetune's `nested` branch, for this package or for tune. tune#917 is open with the same code. The function takes a `tune_results` and reads its per-resample metrics. A `nested_results` holds no per-resample inner metrics (`.inner_metrics` is tune's summarized table, D-043), so the correction cannot run on the nested record. The final fit keeps its tuning run whole, so it can run there, on a number D-014 keeps off the package's surface.
**Decision:** nestedtune does not export the function. tune is its home: the input is tune's class, the per-resample table is tune's, and this package delegates to tidymodels rather than reimplementing (D-002, Conventions). Surfacing the corrected flat estimate beside the nested one waits. `cairn/references/tibshirani2009.md` records the regression case as a conjecture, an over-shoot for unstable learners at p ≫ n, and no validation for a tuned workflow, which is what GP5 declines to ship. Considered and rejected: a `tune_results` method here (a tune function in another package's namespace), and a `nested_final_fit` method (it turns the stored tuning run into a claim, against D-014's posture, before the estimate is settled).
**Consequences:** the reply on #95, left to the maintainer at the plan gate, says so and points at tune#917. A candidate row holds the follow-on: once tune exports the function, a milestone can call it on the final fit's stored tuning run and print the corrected flat estimate beside the nested one as a reading of what nesting bought, with a D-entry trading GP5 and amending D-014's posture at that gate. Falsified by a published validation of the correction for a tuned workflow, or by tune declining the function.

### D-063 (2026-09-14): `compute_metrics()` and `augment()` gain methods on tune's generics for `nested_results` and `nested_results_set`, and `tune::compute_metrics` is re-exported. Extends the tune-generic readers D-052 and D-054 added

**Context:** A run kept under `save_pred = TRUE` holds each outer fold's held-out predictions (D-054), but the only readers were `collect_predictions()` and `collect_metrics()` with the run's own metric set. tune 2.1.0 offers `compute_metrics()` to rescore saved predictions and `augment()` to join them onto the data. M092 found no record of either for a nested run.
**Decision:** both are methods on tune's generics, and `tune::compute_metrics` is re-exported beside `augment`, as D-052 did for `collect_notes()`. `compute_metrics()` scores each fold's saved predictions through yardstick without redoing the inner selection, and its `event_level` defaults to the level the run recorded, not to tune's `"first"`, so a rescore with the run's own metric set gives `collect_metrics()` back. `augment()` joins prediction columns only and refuses an outer design that holds a row out other than exactly once. tune averages repeated predictions, and that averaging needs an oracle for class probabilities first. The set methods stack the element call through `stack_set()`. Considered and rejected: tune's `"first"` default, which rescores a `"second"` run at the wrong level, and a same-named `conf_mat_resampled()`, which masks tune's plain function.
**Consequences:** `augment()` on a repeated or Monte Carlo design waits on the `summarize = TRUE` candidate row. Falsified by a user who needs `augment()` on such a design, or by tune changing either generic's contract.

### D-064 (2026-09-16): IP2's text names the randomness it binds, the clause D-011's Consequences and the reproducibility help template already carry

**Context:** IP2 in `cairn/DESIGN.md` promises the same result for the same seed across worker counts and serial or parallel execution, and disclaims identity across R versions, platforms, and tune versions, but does not say which randomness it covers. D-011's Consequences paragraph states the bound (randomness flowing through R's RNG; kernlab's SVM and keras/torch outside it under any R-side scheme), and RR01 finding B4 (`cairn/reviews/archive/RR01-rng-streams-outer-folds.md:361-367`) asked that DESIGN.md say so beside IP2's existing disclaimers. The help template `man-roxygen/section-reproducibility.R` carries the same sentence as its last paragraph.
...
### D-067 (2026-09-21): the workflow identity keeps comparing a function by its text alone, and `codetools` does not join Imports — supersedes D-066

**Context:** M105 set out to make the identity record the values a function-valued setting reads, so the final fit would refuse a workflow whose function computed something else. D-066 chose `codetools` as the reader of those names. The milestone failed review three times, always on the same criterion, and RB08's review (`cairn/reviews/archive/RR08-closure-identity-stability.md`) answered why. Of the seven failure mechanisms found, two are not properties of the function at all. Enclosure mutation is a property of when the closure last ran, and a data-masked name under non-standard evaluation is a property of what the callees do with the names the code writes down. No recorder of live values decides either from the function, and both refuse a user their own workflow in a check with no bypass.
...
### D-069 (2026-09-21): the six orchestrators are S3 generics on `object`, with a `workflow` method keeping today's signature and a `model_spec` method in tune's argument order. Extends the orchestrator family D-010 named, and keeps D-042's `control` through the dots

**Context:** M107 lets a user call an orchestrator as they call tune, `tune_grid(spec, preprocessor, resamples)`. tune 2.1.0 and finetune 1.3.0 do this with a generic `(object, ...)` and a `model_spec` method `(object, preprocessor, resamples, ...)` that refuses a missing or wrong preprocessor, builds a workflow, and runs the workflow path.
**Decision:** `nested_tune_grid()`, `nested_tune_bayes()`, `nested_tune_race_anova()`, `nested_tune_race_win_loss()`, `nested_tune_sim_anneal()` and `nested_fit_resamples()` become generics `(object, ...)`. The `workflow` method keeps each function's current formals. The `model_spec` method takes `(object, preprocessor, resamples, ...)` followed by the workflow method's named arguments, accepts a formula or a recipe only, and calls the generic again on `workflows::workflow(preprocessor, object)`, so every later refusal names the function the user called. A missing or wrong preprocessor raises `nestedtune_bad_preprocessor`. A formula or recipe given beside a workflow, by name or in the `resamples` position, raises `nestedtune_preprocessor_with_workflow`. A default method refuses any other `object` and names both accepted inputs. `workflow_variables()` is refused as a preprocessor, because tune's own method accepts it and then adds nothing to the workflow. Considered and rejected: a named `preprocessor` argument after the dots on the one function, which ports tune code only after a rename, and a shared internal body with a `call` argument, which moves all six bodies to keep a call the error header prints the same way.
**Consequences:** `nested_final_fit()` stays a function of a workflow (D-041), and a bare spec there is refused with a message naming `workflows::workflow()`. Code reading an orchestrator's formals reads the `workflow` method's. Falsified by the generic breaking a documented workflow call, or by tune changing its `model_spec` argument order.

### D-070 (2026-09-21): tune refuses `workflow_variables()` as a model-spec preprocessor, and a workflow refuses a preprocessor in three positions. Corrects D-069's reason for the first refusal and its statement of the second

**Context:** D-069 says tune's own `model_spec` method accepts `workflow_variables()` and then adds nothing to the workflow. M107's claim audit read tune 2.1.0's `is_preprocessor()`, which accepts only a recipe or a formula, so tune refuses it at entry. The earlier reason came from the wording of tune's error message, which names "dplyr selectors". M107's review found that D-069 also understates the refusal beside a workflow.
...
### D-072 (2026-09-21): `desirability2 (>= 0.2.0)` joins Suggests for the desirability selection rule, which refuses when the package is absent. Extends the dependency set D-065 last touched

**Context:** M109 adds a fourth rule to `selection_rule()` that selects by desirability2's joint desirability. The package is needed where the rule is built, where an orchestrator starts, and where the final fit selects. Version 0.2.0 changed the default for estimating each goal's limits from the data, so 0.1.0 can select a different candidate from the same terms.
**Decision:** desirability2 goes in Suggests with a floor of 0.2.0, the version the tests run on. `selection_rule("desirability", ...)`, the orchestrators given such a rule, and `nested_final_fit()` on a result that recorded one each refuse with class `nestedtune_pkg_not_installed` when it is absent, as the racing tuners do under D-044. Considered and rejected: Imports (every user would install it for one rule), and no floor (a 0.1.0 user gets selections the help does not describe).
**Consequences:** users of the other three rules install nothing new. Falsified by desirability2 changing what `select_best_desirability()` returns for the same terms, which would move the floor.

### D-073 (2026-09-21): `selection_rule()` takes a fourth rule, `"desirability"`, whose terms can name a metric or a tuned parameter. Supersedes D-056's clause refusing a rule outside tune's three and, for this rule, its clause that orderings name tuned parameters only

**Context:** D-056 limited the rule to tune's three selectors and the orderings to tuned parameters. M109 adds desirability2's `select_best_desirability()`, which scores each candidate on terms such as `maximize(roc_auc)` or `minimize(num_comp)`. A term reads a column of the wide metrics summary, so it can name a metric as well as a parameter.
**Decision:** `selection_rule("desirability", ...)` captures its terms as bare expressions, as the orderings are captured, and runs them through `desirability2::desirability()` when the rule is built. That function's errors are raised again with class `nestedtune_selection_rule_terms`. `nested_tune_grid()` and `nested_tune_bayes()` refuse at entry, with class `nestedtune_selection_rule_unknown_term`, a term naming neither a metric in the run's set nor a tuned parameter. With no `metrics`, the set is the one `tune::check_metrics_arg()` gives for the model's mode. The two racing tuners and `nested_tune_sim_anneal()` refuse the rule at entry with class `nestedtune_selection_rule_unsupported`. The folds and the final fit apply it through the one function that applies the other three. The printed label keeps the shape of the other rules: `desirability by <terms>`. Considered and rejected: support under all five tuners (racing drops candidates before the end and needs its own reference), and passing desirability2's unclassed errors on unchanged.
**Consequences:** the other three rules keep D-056's parameters-only orderings. A ROADMAP candidate row holds the three refusing tuners. Falsified by desirability2 documenting its selector for racing or annealing results, or by a term reading a summary column other than a metric or a parameter. (Supersedes D-056's rule clause, and its ordering clause for this rule only.)

### D-074 (2026-09-22): the rolling-origin and sliding-window support covers every orchestrator and `nested_workflow_map()`. Extends D-071, whose bound to what is tested stands

**Context:** D-071 claimed the two outer designs, each with an inner `rolling_origin()`, under `nested_tune_grid()` and `nested_final_fit()` alone. M110 tests the other five orchestrators and `nested_workflow_map()` on the same two fixtures, each against its existing reference.
**Decision:** the support covers `nested_tune_grid()`, `nested_tune_bayes()`, `nested_tune_race_anova()`, `nested_tune_race_win_loss()`, `nested_tune_sim_anneal()`, `nested_fit_resamples()`, `nested_workflow_map()` and `nested_final_fit()`. The final fit rebuilds only the inner design, from its recorded call on the full data, and never reads the outer design. So the final fit on a sliding-window result equals the one on a rolling-origin result with the same inner call. One sliding-window final-fit test, for `nested_tune_bayes()`, stands for the other tuners. No package code changed. Considered and rejected: testing rolling-origin alone under the new orchestrators, which halves the runs but makes the help list which design works where.
**Consequences:** `sliding_index()`, `sliding_period()` and inner designs other than `rolling_origin()` stay unclaimed (M111 and a candidate row). Falsified by a final fit that reads the outer design, or by a user report of a failure on a claimed design and orchestrator.

### D-075 (2026-09-22): `nested_workflow_map()` is claimed on the rolling-origin design alone, and the final fit reads the data but not the outer splits. Corrects D-074's Context and Decision, and supersedes D-071's Consequences clause on the other orchestrators

**Context:** M110's review found three faults in D-074. Its Context says M110 tests `nested_workflow_map()` on both fixtures, and its Decision counts the map under both designs, but the map is tested on the rolling-origin design only. Its Decision says the final fit "never reads the outer design", but the final fit takes the full data from the first outer split. D-071's Consequences still say the other five orchestrators stay unclaimed, and D-074 did not mark that clause superseded.
...
### D-079 (2026-09-23): the Bayes, racing and annealing tuners are tested on the rolling-origin time-series design alone, and four per-tuner test families are dropped where their code path is shared. Supersedes D-074's rejection and narrows D-078's tested set

**Context:** M113 prices the suite at 1276 s serially, with the four `test-time-series-*.R` files at 299 s. D-074 rejected testing rolling-origin alone under the tuners because the help would then list which design works where. D-078 extended the claim to four outer designs under the six orchestrators. The outer design never reaches tuner code: `nested_loop()` hands each tuner one outer fold's analysis frame, and every fixture's inner call is the same `rolling_origin()`.
