...
### D-070 (2026-09-21): tune refuses `workflow_variables()` as a model-spec preprocessor. Corrects the reason D-069 gives for refusing it, and leaves the refusal standing
...
**Decision:** the refusal stands, for the reason that matches tune: the spec route takes what tune's spec route takes, a formula or a recipe, and variables go through a workflow.
**Consequences:** none for the code. (Supersedes the `workflow_variables()` reason in D-069's Decision.)
