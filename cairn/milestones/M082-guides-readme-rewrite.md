...
- [ ] T1: Delete the three why-nest files, their `_pkgdown.yml` row and any `vignette()` or site link naming the page (`grep -rn why-nest vignettes/ README.Rmd _pkgdown.yml`); confirm `nnet` is not in `DESCRIPTION` (it is not at `c5638cf`).
...
- [ ] T6: Rewrite `articles/parallel.Rmd` (start daemons, same call, same result, when it pays) and `README.Rmd` to the AC5 shape; re-knit `README.md`.
