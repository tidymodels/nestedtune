...
- [x] AC6: `nested-cv.Rmd` and `estimate.Rmd` each say in one paragraph that the estimate describes the tune-and-fit procedure, that it is the number to report, and that the final model has no performance number of its own (IP3's documentation obligation). (RB tripwire: ip-touching)
- [x] AC7: `NEWS.md` keeps its `# nestedtune 0.0.0.9000` heading; every bullet is one or two sentences naming one user-visible behavior; the sentence `tests/testthat/test-id-columns.R:230` pins is kept verbatim; `devtools::check()` reports 0 errors, 0 warnings, 0 notes, and `test-id-columns.R` passes.
...
- 2026-09-10: review: AC1-AC7 verified with fresh evidence at `0c9c636`, consistency gate clean, three reviewers ran (17 + 0 + 2 findings), ten findings fixed on the branch and eight rejected with reasons (Review section); the AC1 line locator (`:133` for a vector at `:131`) is a plan-text slip put to the user at the gate.
- 2026-09-10: amendment at the review gate, user chose it over a formal return: AC1 — "the shipped-vignette vector at `benchmarks/sweep-vignette-idioms.R:131` names the four guides" (locator `:133` corrected to `:131`; the check is unchanged).
- 2026-09-10: step-7 approval: m082-guides-readme-rewrite approved for merge.
- 2026-09-10: user override at the gate: issue #91 is commented on after the merge but not closed, pending a human read of the writing quality; the PR body carries `Refs #91` in place of the `Closes #91` line the Resolves slot would have added.
...
Fresh evidence gathered 2026-09-10 at `0c9c636`, the branch at main's head (`2f2c126`) with no sync needed.

- AC1: `ls -R vignettes/` lists `estimate.Rmd`, `nested-cv.Rmd`, `results.Rmd`, `tuners.Rmd` and `articles/parallel.Rmd` and nothing else; `grep -rn why-nest` over `vignettes/`, `README.Rmd`, `README.md`, `_pkgdown.yml` and `NEWS.md` finds no hit; `_pkgdown.yml`'s articles block names the four guides and `articles/parallel` only, all present; the vector at `benchmarks/sweep-vignette-idioms.R:133` is `c("nested-cv", "estimate", "results", "tuners")`. Pass.
- AC2: prose word counts by the criterion's `awk` command: `nested-cv.Rmd` 1211 of 1500, `estimate.Rmd` 1397 of 1400, `tuners.Rmd` 1262 of 1500, `results.Rmd` 1274 of 1500, `articles/parallel.Rmd` 528 of 700, `README.Rmd` 201 of 350. Pass.
- AC3: grepping the five removed headings over `vignettes/` and `README.Rmd` finds none of "Where this example sits", "Passing a control through", "What differs from calling", "What crosses the wire", "Interrupting and cancelling" or a "Why" heading; `results.Rmd`'s only `event_level`/`eval_time` text is one closing paragraph at `:338-341` pointing at the help page, its heading (`## Two more arguments` on main) gone. The five explanations by sentence extraction: the case for nesting and feature selection appear in full on `estimate.Rmd` only, with one linked sentence on `nested-cv.Rmd:19` and one on `tuners.Rmd:425`; the no-subtraction rule in full on `estimate.Rmd`, one linked sentence each on `nested-cv.Rmd:162` and `tuners.Rmd:338`; the fit-count arithmetic in full on `nested-cv.Rmd`, one linked sentence each on `tuners.Rmd:138` and `parallel.Rmd:163`; the seeding paragraph in full under Reproducibility on `nested-cv.Rmd`, one linked sentence each on `tuners.Rmd:295`, `results.Rmd:161` and `parallel.Rmd:130`. Pass.
- AC4: over the extracted and filtered prose of all seven files, the `--|—|contract|invariant|rests on` grep returns no hit; the `you\|your` count is 0 on six files and 1 on `NEWS.md` (the pinned id-column sentence); every heading listed by `grep -nE '^#+ '` read as a noun phrase (36 headings across the six pages, the NEWS heading a version); `Rscript benchmarks/sweep-vignette-idioms.R` prints `clean` and exits 0. Pass.
- AC5: `README.Rmd` read in order: title with logo and the two badges, one paragraph on what the package does, `## Installation`, `## Example` calling `nested_resamples()`, `nested_tune_grid()`, `collect_metrics()` and `nested_final_fit()`, then a "Learn more:" line and a five-item list linking the four guides and the parallel article; `devtools::build_readme()` re-run leaves `README.md` unchanged (`git status` clean on it). Pass.
- AC6: the flattened prose of `nested-cv.Rmd` and of `estimate.Rmd` each contain the one paragraph the implement-gate decision fixed, verbatim, saying the number describes the tune-and-fit procedure, is the number to report, and that the deployed model has no performance number of its own. Pass.
- AC7: `NEWS.md` opens with `# nestedtune 0.0.0.9000`, its only heading; bullets joined and sentence-counted, none exceeds two (77 bullets after the gate fixes below); the pinned sentence "read as a fold label only when the design itself carries a column of that name" is present in `NEWS.md` flattened; `devtools::check()` at `0c9c636` (9m 23s, vignettes rebuilt) reports 0 errors, 0 warnings, 0 notes; `test-id-columns.R` passes (156 expectations). Pass.
- Note on AC1's locator: the shipped-vignette vector sits at `benchmarks/sweep-vignette-idioms.R:131`, with `:133` the closing brace of the same `vapply()` expression; the file is unchanged since the plan commit, so the criterion's line number was off by two when written. The vector's content is what the criterion checks, and it holds; the locator slip is put to the user at the gate.
- Consistency gate: `cairn_validate.py` exit 0, all checks PASS, 18 `references staleness` advisories (standing); no principle changed, so `cairn_impact.py` skipped; `devtools::document()` leaves no diff; `README.md` re-knit unchanged; `pkgdown::check_pkgdown()` no problems; NEWS carries the milestone's entry as its top bullet; no new top-level file; `devtools::check()` clean as above.

### Findings

Three fresh-context reviewers: [O] diff-bug 17 findings, [S] blame-history 0 findings (ten removals checked and cleared as intentional), [S] prior-PR-comments 2 findings and 1 note from the archived Review sections; its GitHub walk was blocked by a network failure inside the agent and was re-run from the session: the probe finds one PR with human inline comments (#30, workflow files), and every one of the 62 PRs that touched the modified files has zero human inline comments. Issue #91 read: the guides, README and NEWS remainder is what this milestone covers.

Fix now, committed on the branch before the gate:
- O1 `NEWS.md` "Two site-only articles and two further vignettes are added" named one article and three vignettes, and the second article is the one this milestone removes → bullet now reads "The parallel article and three further vignettes are added".
- O2, O3 `results.Rmd` "refused by every reader but `summary()`" and "the stacking readers ... warn once" both swept in `collect_notes()`, which reads every fold, warns about none and refuses nothing (`R/nested-results-collect.R:140-148`); main's text enumerated the readers → the paragraph now names the five warning readers, `collect_notes()`'s exemption, and the all-failed refusal as those readers plus `nested_final_fit()` except `summary()` and `collect_notes()`.
- O4 `estimate.Rmd` "biased upward" gives the wrong direction for a loss metric such as RMSE → "biased toward looking good".
- O5, O6 `estimate.Rmd` the Tibshirani paragraph gave only the 0.384 cell and dropped main's two caveats (the SVM 0.475 and tree 0.498 in the same cell; the paper's own remedy is a cheap correction, not nesting) → both restored in two sentences, per `cairn/references/tibshirani2009.md`; four filler clauses cut elsewhere on the page to hold the cap (1396 of 1400).
- O7 `NEWS.md` the `attr(x, "metrics")` half of a dropped bullet had no successor (`R/nested-tune-grid.R:156`) → one sentence added to the procedure-attribute bullet.
- O8 `NEWS.md` `tidymodels` joining Suggests (D-053) was no longer announced while every other Suggests addition is → one bullet added.
- O9 `nested-cv.Rmd` the case for nesting was made a second time on the final-fit section with a second link → the second instance cut to the object's own fact, no argument and no link; the page's opening sentence is the one linked sentence.
- O11 `nested-cv.Rmd` "Each draws a tuning seed and an outer-fit seed for every fold" was wrong for `nested_final_fit()`, which draws one pair (`R/nested-final-fit.R:150-157`) → the paragraph names each function's draw.
- O15 `tuners.Rmd` "the name of one tuning function" understated `fn`, which accepts `nested_fit_resamples()` too (`R/nested-workflow-map.R:28`) → "the name of one orchestrator".
- P2 `tuners.Rmd:155` "shows here as a fold whose `.iter` stops short" pointed at a demonstration the milestone deleted → "shows in this column as".
After the fixes: AC2 counts 1198 / 1396 / 1263 / 1286 / 528 / 201, the AC4 grep and you/your counts unchanged, the sweep `clean`, the four edited pages re-rendered by `rmarkdown::render()`, NEWS at 77 bullets none over two sentences. `devtools::check()` ran at `0c9c636`, before these prose-only edits; the PR's CI check runs on the pushed head.

Rejected, with reason:
- O10 `parallel.Rmd:129-133` seeding pointer "two sentences": the second sentence ("So the serial run under the same seed is the same procedure on the same resamples") is the article's own conclusion, not the seeding explanation; the pointer is one sentence.
- O12 the help-page example fold counts and grid: M081's help-page bullet describes the examples as rewritten whole, which supersedes the earlier example-speed bullet.
- O13 the `std_err` and estimand help-page bullets: covered by the same "Every help page is rewritten" bullet.
- O14 the AC1 locator: recorded above as a plan-text slip, not a diff defect; put to the user.
- O16 `.github/ci-usage-baseline.md:40` names `why-nest-sim.R`: a record of a past CI run, outside AC1's scope.
- O17 `README.Rmd` attaches tidymodels unguarded: README is `.Rbuildignore`d and was unguarded on main; D-053's guard names the pages under `vignettes/`.
- P1 `results.Rmd` the partial-summary warning claim without an executed chunk: T5 called for the failed-fold section cut to the run, the notes and one paragraph; the paragraph's claims were verified against `R/` and the seven test files naming the class, and corrected under O2/O3.
- P3 the README memory table removed: AC3 requires the Why section and its table gone; the table lives at `?nested_resamples`.
