...
- **Branch/PR:** m116-selecting-metric, https://github.com/tidymodels/nestedtune/pull/131   <!-- owner: implement (branch) / review (PR URL) · create -->
...
- 2026-09-24 (pass 3): synced, `origin/main` is an ancestor of the branch; no PR exists. Code changed since pass 2 (T4), so every box was unticked and is re-verified below against this pass's evidence.
- AC1 (pass 3): `test-selection-metric.R` passes in the full suite (below), covering the orderings, `metrics = NULL` on regression and classification, the four other tuners, `procedure_tuner()`, and no entry on fit_resamples. The entry is now resolved at `nested_loop()`'s top through `check_metrics_mode()`, and the final fit still writes its own `metric_name`. The two named sets run on regression alone, as recorded at pass 1.
- AC2 (pass 3): snapshots pin the line on all four surfaces under `"best"` and `"one_std_err"`. Placement and every named absence (desirability, fit_resamples, no entry, on each surface) are asserted, and presence in the all-failed results summary and set. The run whose folds would all fail on an unsuitable set is now refused at entry, so no record lacks the entry. Full suite green (below).
- AC3 (pass 3): both summaries carry `first_metric`, asserted as the record's entry and as `NULL` on untuned runs. The four named help pages describe the line or the entry, with the pass-2 absences added to the final fit's two pages and the results summary's line paragraph.
- AC4 (pass 3): the `...` entry in `R/nested-workflow-map.R` states the first metric chooses under the three rules and every metric is scored, and links `nested_tune_grid()` for `NULL` and desirability; `man/nested_workflow_map.Rd` holds it, and `document()` leaves no diff.
- Evidence base for pass 3: full suite under NOT_CRAN=true after the last code change (`57b89b4`), 89 files, 992 tests, 0 failed, 0 errors, 0 skipped; later commits touch NEWS.md and cairn/ only. `devtools::document()` no diff; six gating sweeps `clean`.
