...
- Gate dispositions (maintainer, 2026-09-14):
  - Fixed now: O1, O3, O4, O7, O8, O12. The final-fit message and both `?nested_final_fit` sentences now limit the set case to a workflow chosen before seeing the set's estimates. The three help sentences say "the results object the fit was built from" again. The "What to report" sentence splits the two cases. The plot test is renamed, the long roxygen lines are rewrapped, and the NEWS bullet is reflowed and names the condition. The test's `set_rows_wording`, the hand-agreed print constant and 4 snapshot messages carry the new sentence.
  - Rejected: O2 and O10, because the plan gate chose the 67-character subtitle line and a subtitle has no room for the condition. O5 and O6, because the plan gate chose one static message over recording the workflow id. O9, because the multi-line NAMESPACE form is what `document()` writes here under roxygen2 8.1.0, and the candidate row asked for it to be committed. O11, because it mirrors `?nested_tune_grid`.
- Post-fix re-verification: the plot test file passes (270). Under `NOT_CRAN=true` the final-fit print snapshots differed only by the new sentence in 4 places, and were accepted after that diff was read. The full suite and `devtools::check()` re-run on the fixed tree. All six gating sweeps exit 0, `air format --check` is clean, AC1's grep prints nothing and AC6's command prints 0.
- AC7 re-run on `62006a5`: `devtools::test()` 10116 passed, 0 failed, 0 errors, 0 skipped. `devtools::check()` 0 errors, 0 warnings, 0 notes. `document()` leaves no diff.
