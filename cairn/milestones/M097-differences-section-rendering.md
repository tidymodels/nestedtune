...
- AC6: at `53a0630` on a clean tree, `devtools::document()` left the tree clean, and `devtools::test()` passed 10140 assertions with 0 failures, warnings or skips. All six commands `--list-gating` prints exited 0 at `262b154`, and no file they read (`R/`, `man-roxygen/`, `vignettes/`, `README.Rmd`) changed after that commit. Pass.
