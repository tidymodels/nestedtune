...
- [x] AC4: `Rscript -e 'devtools::check()'` reports 0 errors, 0 warnings, 0 notes.
...
- 2026-09-11 AC4: `devtools::check()` at 630a822: 0 errors, 0 warnings, 0 notes, 10m50s. The fix-now commits after it change the yaml (not in the tarball), a test comment, and `cairn/`, and the test file passes 34 after the edit. Verified.
- 2026-09-11 consistency gate: `cairn_validate` all checks passed (18 staleness advisories, pre-existing). `devtools::document()` no diff. `README.Rmd` unchanged against main, so README.md is in sync. `pkgdown::check_pkgdown()` no problems. NEWS: no user-visible change, no entry owed. `.Rbuildignore` covers `.github` already. The six sweeps clean at 630a822. `ci-usage.py`'s `read_paths_ignore()` lists `prose-sweep.yaml` with the five others (its run fetch failed on a TLS timeout, unrelated).
- 2026-09-11 reviewers: [S] blame-history found no contradiction (its note on the PROFILE line reflow is the rulebook's 120-line cap on that file). [S] prior-review found no prior-review evidence on the touched files (PR #30's human threads touch other workflows). [O] diff-bug returned 12 findings, triaged below.
- 2026-09-11 finding 1 (steps stop at the first hit, so a red run names one mode): fix now, `if: ${{ !cancelled() }}` on steps 2-6.
- 2026-09-11 finding 2 (an open PR gets two runs per push, on the branch ref and the merge ref): reject, deliberate and recorded (work log, yaml comment). A PR lives from the merge gate to the merge, so the doubled cost is bounded to that window.
- 2026-09-11 finding 3 (the yaml comment says the workflow "gates" a PR though no status check is required): fix now, the comment names the profile's merge clause.
- 2026-09-11 finding 4 (PROFILE's hang-cap list and divergence tags omit the new workflow): fix now, both lists name it.
- 2026-09-11 finding 5 (`ci-usage-baseline.md` not re-measured for the new run population): reject, the baseline is a dated record and the plan gate's falsifier is a promotion condition read at a later measurement, not owed here.
- 2026-09-11 finding 6 (`verify` names only the `--plain` modes): reject, AC3 as planned. The other four modes gate at review and in CI.
- 2026-09-11 finding 7 (long lines from the PROFILE reflow): reject, style under a line cap.
- 2026-09-11 finding 8 (the `setwd` restore's safety under parallel files unstated): fix now, one comment.
- 2026-09-11 finding 9 (the test hardcodes the page list the script owns) and finding 10 (no check that the three surfaces name the same six invocations): follow-up, absorbed into the existing `sweep-prose.R` candidate row.
- 2026-09-11 finding 11 (the timing comment names a run, not its job): fix now, job 103440877732 added.
- 2026-09-11 finding 12 (AC boxes unticked, a stray blank line in the work log): no change, the boxes tick as evidence lands and the blank line is history.
