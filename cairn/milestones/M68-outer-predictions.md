...
- [ ] T3: Tests first for `.extracts`: the oracle on `f(fitted$.workflow[[1]])`, the erroring `f` (fold completed, `NULL`, the `"outer extract"` note row through `own_note()`), absence under `extract = NULL`. Then the extract call inside `nested_fold_fit()` after the outer fit, in its own `tryCatch()`, and the column plumbing of T1 for `.extracts`.
