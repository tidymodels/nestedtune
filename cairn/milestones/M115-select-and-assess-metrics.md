...
- 2026-09-24: T1 done. The new block replaces the file's "selects under the metrics it was given" block, whose one assertion it repeats, because the helper closure's cache key made the reference loop build twice (the M42 candidate row's gap). A planted `[[2L]]` for the first-metric lookup turned 9 checks red. Full suite 0 failures, 11406 passes.
- 2026-09-24: T2 done. Own `@param metrics` on `nested_tune_grid()`, inherited by the Bayes, race and annealing pages, and a one-statement one on `nested_fit_resamples()`; a references entry on the grid page that the other three inherit; a source comment at the first-metric lookup citing the four `first_metric()` calls.
