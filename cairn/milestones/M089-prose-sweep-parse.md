...
7. **A new code comment states a reason that does not hold.** `benchmarks/sweep-prose.R:313-314` says the YAML drop means a `---` inside the header never opens a fence, but the fence test is `^\s*``` `, which `---` cannot match with or without that drop. The guard on line 316 is real and does other work; the sentence misexplains it. Recommended: fix now.
