...
- [x] AC1: `Rscript benchmarks/sweep-prose.R --plain` reports every prose sentence of the six pages matching a plain clause, as `file:line: <clause name>: <text>` with `line` the line the sentence starts on, and exits 1 on any hit; at the milestone's head it prints `clean` and exits 0.
- [x] AC2: `Rscript benchmarks/sweep-prose.R --plain --pages <path>` (`--pages` replacing the page list; a page read with or without a YAML header) run on a file holding only the reference intro prints `clean` and exits 0, the file's text byte-identical to `git show 0d8611e:vignettes/nested-cv.Rmd | sed -n '17,33p'`.
- [x] AC3: `tests/testthat/test-sweep-prose.R` runs `--plain --pages` over a committed fixture under `tests/testthat/fixtures/` whose header comment names it hand-authored with no generator or seed, and which carries: one sentence per plain clause, at least one of them in mixed case and one wrapped across two lines; one sentence holding every clause's marker only inside backtick spans; and one near-miss sentence per clause (`maybe`, `shouldering`, `has not been`, `, the tuning`, a possessive `'s`) — and asserts that the output names each clause exactly once by clause name with the wrapped sentence reported at its first line, and names the backtick and near-miss sentences not at all. The test skips with the reason `sweep-prose.R not in the source tree` when `benchmarks/sweep-prose.R` is absent from the package root testthat resolves.
- [x] AC4: `Rscript benchmarks/sweep-prose.R` and `Rscript benchmarks/sweep-prose.R --spans` print `clean` and exit 0 over the six pages, and `wc -w` on each of the six files is at most 110% of its value at the branch point.
- [x] AC5: one fresh-context reading of the six pages per review pass in the SimpleEnglish skill's check mode (each entry a rule number quoted from the skill's `references/rule-catalog.md`, the offending text, a rewrite), by a reader withheld this milestone file and the `--plain` output, saved as `cairn/surveys/M086-reader-review.md`; every entry is triaged at the review gate as fixed or rejected with a reason, and the report is never rerun after fixes within a pass.
- [x] AC6: `Rscript -e 'devtools::check()'` reports 0 errors, 0 warnings, 0 notes; `README.md` is re-knitted from `README.Rmd` with no further diff from `devtools::build_readme()`; `NEWS.md` carries one bullet for the rewrite.
...
- 2026-09-11: PR #98 opened; format-suggest red on the new R files, fixed by `air format` (whitespace only, sweeps and fixture test re-run clean), pushed.
## Decisions

## Review

- 2026-09-11 pass 1, at e648dd9; `main` unmoved since the branch point (fetch clean, nothing unpushed).
- AC1: `Rscript benchmarks/sweep-prose.R --plain` printed `clean`, exit 0 over the six pages; the hit format and exit 1 on a hit are exercised by AC3's fixture run (six `file:line: <clause>: <text>` lines, status 1). Verified.
- AC2: the reference intro extracted by the stated `git show ... | sed -n '17,33p'` into a headerless scratch file (`cmp` byte-identical), `--plain --pages <file>` printed `clean`, exit 0. Verified.
- AC3: `devtools::test(filter = "sweep-prose")` 13 expectations pass, 0 fail; the fixture's header names it hand-authored with no generator or seed, one sentence per clause with `MAY` in mixed case and that sentence wrapped over two lines, one backtick-span sentence, one near-miss sentence carrying `Maybe`, `shouldering`, `has not been`, `, the tuning` and `reader's`; the test asserts each clause name exactly once, the wrapped sentence at its first line, silence on the span and near-miss sentences, and skips with `sweep-prose.R not in the source tree`. Verified.
- AC4: default and `--spans` sweeps printed `clean`, exit 0; `wc -w` head against b84cbcb: nested-cv 1800/1798, estimate 1405/1408, tuners 2184/2187, results 1837/1836, parallel 801/804, README 370/367, all within 110%. Verified.
- Fix-now commit 8f21cb1 re-verified: `--plain`, default and `--spans` sweeps `clean` exit 0; the reference intro `clean`; fixture test 13 pass (fixture now six near-miss sentences, one per clause, listed in the header); `wc -w` head against b84cbcb: nested-cv 1809/1798, estimate 1405/1408, tuners 2184/2187, results 1837/1836, parallel 801/804, README 370/367, all within 110%; `build_readme()` no diff.
- AC5: fresh Opus reader (withheld this file, the surveys directory and the `--plain` output) given `cairn/surveys/M086-reader-prompt.md` unchanged; report `cairn/surveys/M086-reader-review.md`, pass 1, 78 entries (nested-cv 21, estimate 18, tuners 14, results 14, parallel 7, README 4). Triage, by rule class, every entry covered: rule 9.4 since→because (6: estimate 9, 14; tuners 7; results 12; parallel 3; README 1) fixed; rule 1.14 spelling (2: estimate 11, tuners 13) fixed; rule 3.4 present perfect (results 13) fixed; GR-3 pronoun referents (2: nested-cv 11, results 6) fixed; rule 3.6 passive voice (34) and rule 3.5 -ing verb forms (15) follow-up, absorbed into the #91 candidate row (the plain clauses never bound either rule, and the row already carries the naming-the-actor item); rule 6.3 25-word cap (8) rejected, the plan gate kept the 30-word cap and every sentence passes it; rule 6.6 paragraph length (3), rule 4.3 list shape (2), rule 9.3 phrasal verbs (3), GR-4 (nested-cv 5) and rule 5.4 (estimate 16) rejected as style outside the plain clauses. Not rerun after the fixes. Verified.
- AC6: `devtools::check()` at 8f21cb1 (the fixed tree): 0 errors, 0 warnings, 0 notes, 6m43s; `build_readme()` leaves `README.md` unchanged; `NEWS.md` carries the one rewrite bullet. Verified.
- Consistency gate: `cairn_validate.py` all checks passed (18 advisory reference-staleness warnings, pre-existing); no principle changed, impact report skipped; `devtools::document()` no diff; `pkgdown::check_pkgdown()` no problems; NEWS entry present; no new top-level file; `.Rbuildignore` covers `benchmarks`.
- Independent review, three lenses. [O] diff-bug: (1) stripped backtick spans joined their neighbours into fabricated hits (`has \`x\` been` reported `has been`) — fixed, plain mode leaves a mark per span; (2) reader report absent at spawn time — superseded by AC5 above; (3) nested-cv.Rmd:282 ranking sentence read as a capability claim (also [S] blame-history 1) — fixed; (4) `--pages` with no path ran over zero files and printed `clean` — fixed, now an error; (5) all near-miss markers in one fixture sentence — fixed, one sentence per clause, AC3 read strictly; (6) NEWS bullet omitted `have been` and called phrases words — fixed; (7) `--plain` ignored beside `--spans`/`--openings`/`--paragraphs` — rejected, the modes are exclusive by the script's design and no page run combines them; (8) parallel.Rmd:111 oversubscription stated as a fact — rejected, the sentence states what the configuration does and is the reason the inner tuning is serial; (9) slop pattern escapes only `.` and `/` — rejected, latent, no phrase carries another metacharacter; (10) list-item lines not swept — rejected, the existing candidate row on the script's prose definition; (11) fixture path interpolated into a regex — rejected, latent. [S] blame-history: (1) as [O] 3, fixed; (2) estimate.Rmd:59-62 modal removal flattened counterfactuals — rejected, "does not say what a different training set gives" is the same claim in the simple tense, the plan gate chose this; (3) AC5 pending — superseded; (4) parallel.Rmd:17 "can make it finish sooner" — rejected, true and plain; (5) systemic modal removal — noted, the milestone's purpose; (6) `--pages` and `--roxygen` — no regression. [S] prior-review: (1) nested-cv.Rmd:354 IP3 paragraph edited (`carrying` → `with`) — rejected, a one-word comma-ing repair with the meaning intact, the M082 fixed text's substance unchanged; probe found one human inline comment on PR #30 (pkgdown.yaml), none on the touched files.
