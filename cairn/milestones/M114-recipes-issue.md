...
Status: draft. Nothing is posted before the user approves this text at the M114 review gate.
...
The per-call difference is about 12 times:
...
We measured with recipes 1.4.0, purrr 1.2.2, cli 3.6.6 and R 4.6.1 on macOS:
...
tune calls `tune_args()` often during a search. In our package's tests, two Bayesian-search fixtures changed from bare names to strings. Two test files that use them went from 57.6 s and 59.9 s to HEAD_FIGURES.
