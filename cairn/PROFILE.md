...
- Every gating prose sweep clean: run each command `Rscript benchmarks/sweep-prose.R --list-gating` prints. The script owns that list. `tests/testthat/test-sweep-prose.R` reads it from there. `prose-sweep.yaml` keeps one step per line as a copy checked by hand against it.
...
- Divergences from that stock shape (M11 ×2, M12 rev. M31, M14, M33, M52, M78, M80, M88). **A `concurrency` block** cancels a
  ...
  hunts the hang on demand under a job cap far above the rest, invisible to `ci-usage.py`. **A devel-vctrs leg** (`devel-vctrs.yaml`, M80) runs the suite against
  vctrs from `r-lib/vctrs@main`, watching the experimental `vec_cbind_frame_ptype()` `R/nested-results.R` has a method
  on; non-gating. Triggers: `push`, `pull_request`, weekly `schedule` (Monday 06:00 UTC), `workflow_dispatch` (M102);
  `cache-version: devel-vctrs` keys the library cache it saves (development vctrs inside) apart from every other
  workflow's. **Three organization workflows** ride unedited at tidymodels' shared blobs (`lock.yaml`,
  `pr-commands.yaml`, `format-suggest.yaml`, M33): no `push`/`pull_request` trigger, so neither the filter nor
  `ci-usage.py` sees them; `format-suggest.yaml` runs `air format .` (see DESIGN).
