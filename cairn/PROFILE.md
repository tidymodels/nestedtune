...
- Every gating prose sweep clean: run each command `Rscript benchmarks/sweep-prose.R --list-gating` prints. The script owns that list. `prose-sweep.yaml` and `tests/testthat/test-sweep-prose.R` read it from there.
