...
- Divergences from that stock shape (M11 ×2, M12 rev. M31, M14, M33, M52, M78, M80, M88). **A `concurrency` block** cancels a
  ...
  step higher on windows. Only its macos-latest leg builds and checks the vignettes, which `R-CMD-check-hard.yaml` also checks and `pkgdown.yaml` knits (M112, D-076).
