...
- :155 "The `.iter` column says which stage each candidate came from" — c e
- :202 "A race's `.inner_metrics` looks complete but is not" — e
- :303 "A tuned procedure, the whole resample-tune-select-fit sequence" — d
- :341 "Read the table fold by fold rather than as one difference" — b d
- :356 "A comparison across model families needs every family scored" — d e
- :389 "The baseline has nothing to tune" — b
- :424 "The parameters view keeps the outer folds on the x axis" — c
