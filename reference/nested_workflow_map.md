...
- ...:
  ...
  instead. A set that does not suit the model of any workflow the map
  runs is refused before any workflow runs, a workflow routed to
...
class is another. The workflows before it have run by then. A metric set
in `metrics` that does not suit the model of any workflow the map runs
is the exception: it is refused before any workflow runs. What is raised
is the original condition object, with `Workflow "<id>": ` written in
front of the first line of its message and this function, or the reading
