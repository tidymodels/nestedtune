...
every orchestrator shares, `param_info`, `event_level`, `eval_time` and
`select`, the
...
`seed` left out. See "Differences from calling tune directly" on each
orchestrator's help page for what those slots are.
