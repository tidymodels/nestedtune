...
- [`summary()`](https://rdrr.io/r/base/summary.html) of a nested result,
  ...
  as `Selecting metric: rmse` prints under the `"best"`, `"one_std_err"`
  and `"pct_loss"` rules, the default included. It is absent under the
  `"desirability"` rule. The record that
  [`extract_procedure()`](https://nestedtune.tidymodels.org/reference/extract_procedure.md)
  returns holds the name as `first_metric`, and the summaries carry it
...
- A `nested_final_fit` now answers
  ...
  and
  [`outcome_names()`](https://tune.tidymodels.org/reference/outcome_names.html)
  with what the same call gives on its trained workflow.
  [`extract_recipe()`](https://hardhat.tidymodels.org/reference/hardhat-extract.html)
  takes `estimated`. Apart from
  [`extract_recipe()`](https://hardhat.tidymodels.org/reference/hardhat-extract.html),
...
- [`collect_metrics()`](https://tune.tidymodels.org/reference/collect_predictions.html)
  ...
  [`collect_metrics()`](https://tune.tidymodels.org/reference/collect_predictions.html)
  does. A summarized table holds each metric’s mean, and an unsummarized
  one holds each outer fold’s estimate. The fold labels, `.eval_time`
  and `wflow_id` stay as key columns. An unknown `type` is refused with
  class `nestedtune_bad_type`. A metric scored with two estimators, or
  named like a key column, is refused with class
  `nestedtune_wide_collision`.

- [`autoplot()`](https://ggplot2.tidyverse.org/reference/autoplot.html)
  on a `nested_results` or a `nested_results_set` takes `metric` and
  `eval_time`, which choose the panels of `type = "performance"`. A
  static metric’s panel is drawn whatever `eval_time` names, as in tune.
  A metric or a time the run did not score, or either argument with
  `type = "parameters"`, is refused with class
  `nestedtune_bad_plot_filter`.

- [`predict()`](https://rdrr.io/r/stats/predict.html) on a
  `nested_results` or a `nested_results_set` now refuses with class
  `nestedtune_predict_results` and names
  [`nested_final_fit()`](https://nestedtune.tidymodels.org/reference/nested_final_fit.md),
...
- The workflow identity every result records now holds a case-weights
  ...
  them.
  [`nested_final_fit()`](https://nestedtune.tidymodels.org/reference/nested_final_fit.md)
  refuses a workflow that differs in either, with class
  `nestedtune_workflow_mismatch`, and the message names the part: the
  column, the adjustment and its argument, or one side carrying the part
  and the other not. A workflow with neither keeps the identity it had,
  so a results object saved before this change is accepted under the
  workflow it ran under. `tailor` joins Suggests for the tests.
...
- The coverage job no longer fails after every test passed. Daemons
  ...
  file a daemon was killed in the middle of writing.

- Every help page now passes the same plain-English sweep as the guides:
  no semicolon, no contraction, no `has been` or `have been`, none of
  the words should, may, might, could and would, no word ending in -ing
  after a comma other than a few prepositions and pronouns, and none of
  the filler words and phrases, in any roxygen prose sentence.

- The four guides, the parallel article and the README are rewritten to
  pass a plain-English sweep: no semicolon, no contraction, no
  `has been` or `have been`, none of the words should, may, might, could
  and would, no word ending in -ing after a comma other than a few
  prepositions and pronouns (including, during, nothing), and none of a
  list of filler words and phrases, in any prose sentence.
