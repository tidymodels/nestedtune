...
# AC3: the selection-rule line holds its shape on both prints
...
---
    ...
    Message
      ...
      i No tuning ran: the workflow was fitted as given on every row.
        `extract_workflow()` returns the fitted workflow; `extract_tune_results()`
        and `extract_scored_candidates()` have nothing to return and refuse.

---

    Code
      print(summary(untuned))
    Message
...
# AC3: the two-ordering line holds its shape on both prints
