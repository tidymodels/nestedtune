...
#' with [tune::last_fit()], skipping the inner stage, since there is nothing
#' to search. It is [nested_tune_grid()] with the inner tuner removed, so a
#' fixed workflow and a tuned one score on identical outer folds and their
#' per-fold metrics join by fold label. That page is the reference for
...
#'   leaves. `.selected` is a zero-row, zero-column tibble on every completed
#'   fold and `NULL` on a failed one. `.inner_metrics` is a zero-row table
#'   with tune's summary columns and no parameter column. `.tuning_seed`
#'   holds the seed the loop drew for the fold's tuning step, consumed by
#'   nothing. There is no `grid` attribute, and the `procedure` record names
...
#' orchestrators that tune; each of those refuses a workflow with no marker,
#' with class `nestedtune_untuned_workflow`, naming this one.
#'
#' @section What the readers answer:
#'
...
#' outer fits produced; [collect_selections()], [collect_inner_metrics()] and
#' [agreement()] return zero rows; `autoplot(type = "performance")` draws the
#' fold scores, where `autoplot(type = "parameters")` refuses with class
...
#' on `resamples$splits[[i]]` under `res$.outer_fit_seed[[i]]`, applied as
#' `?nested_tune_grid` shows, with no tuning line before it.
#'
...
#' [tune::control_grid()] and [tune::control_last_fit()] one class, so any of
#' the three is accepted here as the same object. Every slot of
#' `control_resamples()` falls under one of seven headings, most under the
#' last, because the one tune call a fold makes here is the outer scoring
#' fit, whose own control this package builds.
#'
...
#' passed through to.
#'
#' **Kept from the outer fit: `save_pred`, `extract`.** With `save_pred =
#' TRUE` the result carries a `.predictions` list column, each completed
#' fold's predictions on its assessment rows as [tune::last_fit()] returns
#' them; with `extract` a function, an `.extracts` list column holding the
...
#' [tune::control_last_fit()] at the level and parallelism above, which reads
#' none of these; the workflow's packages are required at entry, and the
#' fitted workflow is reached through `extract`.
#'
