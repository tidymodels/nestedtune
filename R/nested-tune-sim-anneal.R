...
#' it scores `initial` candidates on that fold's inner resamples, then for
#' `iter` iterations perturbs the current candidate, scores the perturbation
#' and keeps it or falls back by finetune's annealing rule, and then selects,
#' finalizes, fits and scores on the outer split as [nested_tune_grid()]
#' does. That page is the reference for everything the orchestrators share,
#' and [nested_tune_bayes()] is this function's nearest sibling.
#'
...
#'   `control` and nothing else, matched by name; what becomes of each slot
#'   is under the section on differences from finetune.
...
#'   `.iter` column after `.config`: `0` for the initial candidates, whose
...
#' its tuning seed, as [nested_tune_bayes()] does, and a `tune_results`
...
#' proposes nothing. finetune stops a fold's search early after `no_improve`
#' consecutive iterations without improvement (`Inf` by default, so never,
#' unless the control sets it) or when `time_limit` is reached; the fold
#' completes with the candidates scored so far, and nothing about the early
#' stop reaches `.notes`.
#'
...
#' started. [finetune::control_sim_anneal()] has no seed slot, so nothing is
#' injected into the control. On the parallel path every daemon's library
#' must hold finetune, which the loop attaches in each daemon before the
#' first fold is sent, warning where it cannot.
#'
...
#'
#' There is no `control` formal, but a [finetune::control_sim_anneal()]
#' passed through `...` as `control` reaches the inner search in every fold
#' and the final fit that re-runs the result: `control = control_sim_anneal(
#' no_improve = 5, verbose_iter = FALSE)`, say, to stop a fold's search
#' sooner and keep the console quiet. What runs is the control passed, or
...
#'
#' **Settable as its own argument: `event_level`.** Set through the argument
#' alone, as on the grid page; a control at finetune's default takes the
#' argument's level, and one naming a different level is refused at entry.
...
#' outside what the seeds can promise. `verbose_iter`, `TRUE` in finetune's
#' default, prints the annealing log from every fold of a serial run, one
#' log per fold, and from a mirai daemon where nothing shows it; pass
...
#' the grid page describes, `parallel_over` included. This classification
#' was read on finetune 1.3.0; the version that added `workflow_size` to
#' `control_sim_anneal()` is not named in finetune's NEWS, and the
#' `>= 1.0.1` floor this package declares does not require it.
#'
#' **Kept from the outer fit: `save_pred`, `extract`.** Both reach the outer
#' fit, whose predictions and extracts come back as `.predictions` and
#' `.extracts` (the grid page has the shape); the inner search's are still
#' discarded.
#'
...
#' of the process that tuned (a daemon's own on the parallel path), every
#' fold overwriting the last one's; nothing of it reaches the result.
#'
