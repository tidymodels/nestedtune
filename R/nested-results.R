...
new_nested_results <- function(
  ...
  # The outer fit's extract and predictions, each a column only when the
  # control asked for it (M68): the slot is read off the recorded procedure,
...
#'   workflow-set run
#' @export
collect_metrics.nested_results <- function(
  x,
  ...,
  summarize = TRUE,
  type = c("long", "wide")
...
pivot_metrics_wide <- function(long) {
  ...
  keys <- long[setdiff(names(long), dropped)]
  group <- if (length(keys) == 0L) {
    rep(1L, nrow(long))
  } else {
  ...
  cols <- as.list(keys)
  cols <- lapply(cols, function(col) col[first])
  n <- sum(first)
  row <- match(group, group[first])
  for (metric in unique(long$.metric)) {
    at <- long$.metric == metric
    column <- rep(NA_real_, n)
    column[row[at]] <- long[[value]][at]
    cols[[metric]] <- column
  }
  if (length(cols) == 0L) {
    return(new_tbl(list()))
